...
- [x] Refine frontmatter sidebar panel UI
  ...
  - [ ] Ensure properties which are arrays of strings in the schema (or frontmatter) use the tag-input element we defined and check it works properly.
- [ ] Make the frontmatter panel resizeable (using the shadcn `resizabel` component maybe?) Should have sensible min and max widths.
