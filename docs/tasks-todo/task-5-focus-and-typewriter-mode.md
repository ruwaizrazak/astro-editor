...
- [ ] Okay, this works when editing long documents, but it doesn't work well when I'm actually writing a document because it relies on there being sufficient scroll room at the end or beginning of the document. If my document is only one line long, like there's nothing to scroll right, and as I type the document and we get to the middle because there's no more lines underneath it, kind of can't be scrolled. I'm not too worried about it at the beginning, in that I don't mind that we're not in the middle of the document until we get halfway down with our writing. Half the point of this is often going to be when I'm writing at the end of a document, i.e. adding new stuff to something I'm writing, and at the moment that will always be at the bottom of the editor window. Can you just take a step back and think about a way we might solve this again, which isn't super complex. I feel like this is probably going to involve adding some elements I don't know, before or after the thing, but just think about that problem for a moment. Otherwise, everything seems to work pretty much fine.
- [ ] Use the centre icon for focus mode and don't change it out to anything else. The button already shows its current state because it has a background colour when it's toggled on. That's the icon in the │
      │ unified title bar.
- [ ]
- [ ] If both sidebars are hidden, hide the whole contents of the UnifiedtitleBar and statusbar after the user starts typing in the enditor. Only show it again if the user moves their mouse near the top of the screen (or opens either sidebar, obvs). See screenshot of iA writer in this state.
- [ ] Do not highlight the Save button so agressively when the doc isDirty. It means it's always full-on-black when I'm typing in the editor because which is distracting.
